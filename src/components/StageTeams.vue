      ...
      <v-col md="12">
        <v-row no-gutters>
          <v-col md="6">
            ...
            <add-stage-team :groupInfo="currentGroupTeams" />
          </v-col>
            ...
            <v-card>
              ...
              <v-tabs-items v-model="tab">
                <v-tab-item>
                  <v-card flat>
                    <knock-out-standings
                      :currentGroupTeam="currentGroupTeams.name"
                    ></knock-out-standings>
                  </v-card>
                </v-tab-item>

                <v-tab-item>
                  <knock-out-fixtures :groupInfo="currentGroupTeams" />
                </v-tab-item>
              </v-tabs-items>
            </v-card>
          </v-col>
...
import AddStageTeam from "./AddStageTeam.vue";
import KnockOutFixtures from "./KnockOutFixtures.vue";
import KnockOutStandings from "./KnockOutStandings.vue";
...
export default {
  ...
  methods: {
    ...
    stageChange(items) {
      ...
      items.forEach(item => {
        ...
        console.log("Stage Teams", key, data);
  ...
  mounted() {
    ...
    FirebaseService.getKnockOut().on("value", this.stageChange);
  },
  beforeDestroy() {
    FirebaseService.getKnockOut().off("value", this.stageChange);
  }
};
</script>
